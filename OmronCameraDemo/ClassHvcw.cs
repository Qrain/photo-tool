...
namespace OmronCameraDemo
{
    ...
    public unsafe class ClassHvcw
    {
        ...
        /// <returns></returns>
        unsafe public bool Connect(string uuid, string Token)
        {
            if (hHVC == 0)
            {
            ...
            Byte[] byUUID = System.Text.Encoding.UTF8.GetBytes(uuid);
            Byte[] byToken = System.Text.Encoding.UTF8.GetBytes(Token);
        ...
        unsafe public bool GenerateSound(string SSID, string Password, string Token)
        {
            ...
            Byte[] byFile = System.Text.Encoding.UTF8.GetBytes(SoundFile);
            Byte[] bySSID = System.Text.Encoding.UTF8.GetBytes(SSID);
            Byte[] byPassword = System.Text.Encoding.UTF8.GetBytes(Password);
            Byte[] byToken = System.Text.Encoding.UTF8.GetBytes(Token);
        ...
        // ***** Set user name (recognition) *****
        unsafe public int SetUserName(string strUserName)
        {
            int ret;
            byte status;
            ...
            int n = 0;
            Byte[] data = System.Text.Encoding.ASCII.GetBytes(strUserName);
        ...
        unsafe public int GetUserName(string strUserName)
        {
            int ret;
            byte status;
            byte[] userName = new byte[44];
            ...
            if (hHVC != 0)
            {
                fixed (byte* pUserName = &userName[0])
                {
                    ret = stdHVCW_ALBUM_GetUserName(hHVC, 0, pUserName, &status);
                }
                if ((ret != HVCW_SUCCESS) || (status != 0))
                {
                    return HVCW_FAILURE;
                }
                strUserName = System.Text.Encoding.UTF8.GetString(userName, 0, 40);
                return ret;
            }
            return HVCW_NOHANDLE;
        }
        ...
        unsafe public int StartLive()
        {
            ...
            if (hHVC != 0)
            {
                ...
                ret = stdHVCW_StartStreaming(hHVC);
                if (ret == HVCW_SUCCESS)
                {
                    bLive = true;
        ...
        unsafe public int StopLive()
        {
            ...
            if (hHVC != 0)
            {
                ...
                ret = stdHVCW_StopStreaming(hHVC);
                if (ret == HVCW_SUCCESS)
                {
                    bLive = false;
                }
